// Sync utilities for Fireproof database
...
export async function enableFireproofCloudSync(): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      dashboardUrl: `https://fireproof.storage/dashboard/demo-app-${Date.now()}`,
      syncUrl: `https://fireproof.storage/sync/demo-app-${Date.now()}`
    }
...
export async function enableS3Sync(bucketName: string, region: string): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      ...
      syncEndpoint: `https://api.demo-app.com/sync`
    }
