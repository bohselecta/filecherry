...
export async function enableFireproofCloudSync(): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      dashboardUrl: `https://fireproof.storage/dashboard/test-fireproof-${Date.now()}`,
      syncUrl: `https://fireproof.storage/sync/test-fireproof-${Date.now()}`
    }
...
export async function enableS3Sync(bucketName: string, region: string): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      ...
      syncEndpoint: `https://api.test-fireproof.com/sync`
    }
