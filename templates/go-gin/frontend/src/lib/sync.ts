...
export async function enableFireproofCloudSync(): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      dashboardUrl: `https://fireproof.storage/dashboard/{{PROJECT_SLUG}}-${Date.now()}`,
      syncUrl: `https://fireproof.storage/sync/{{PROJECT_SLUG}}-${Date.now()}`
    }
...
export async function enableS3Sync(bucketName: string, region: string): Promise<SyncStatus> {
  try {
    ...
    const connection = {
      ...
      syncEndpoint: `https://api.{{PROJECT_SLUG}}.com/sync`
    }
